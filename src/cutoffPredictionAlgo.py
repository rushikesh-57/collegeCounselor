...
def dataQualityChecks():
    ...
    grouped.to_csv('C:/Work/College Counselor/Cutoff/combined_new.csv', index=False)

    # **1️⃣ Identify New Colleges and Branches added**
    new_entries = grouped[grouped['min'] == grouped['max'].max()]
    new_entries.to_csv('C:/Work/College Counselor/Cutoff/new_entries_new.csv', index=False)
    ...
    college_changes.to_csv('C:/Work/College Counselor/Cutoff/college_changes_new.csv', index=False)
    ...
    branch_changes.to_csv('C:/Work/College Counselor/Cutoff/branch_changes_new.csv', index=False)
...
def cutoffPredictWMA():
    # Sample DataFrames for 5 years (Replace with actual data)
    tableList = ['Cap1_cutoff_2025_Rank_Predicted','Cap1_cutoff_2024_Rank','Cap1_cutoff_2023_Rank','Cap1_cutoff_2022_Rank','Cap1_cutoff_2021_Rank','Cap1_cutoff_2020_Rank']
    dfs = {}
    conn = sqlite3.connect("data.db")
    c = 2025
    ...
    melted_df.to_csv('C:/Work/new/melted_df.csv', index=False)
    ...
    pivot_df.to_csv('C:/Work/new/pivot_df.csv', index=False)

    # melted_df = melted_df[melted_df['Cutoff'] > 0]
    # melted_df.to_csv('C:/Work/melted_df_filtered2.csv', index=False)
    # melted_df.to_sql('Branch_Cutoff_Yearly_Data', conn, if_exists='append', index=False)
